...
<div ng-init="uCommunication.compose();uCommunication.getsession()" class="tab-pane active maildiv" id="maildiv">
	...
	<div class="tab-pane active" id="tab_b" ng-hide="mailread">
		...
		<div class="col-lg-12 content">
			<div class="row" ng-repeat="inbox in notificationlist" ng-cloak>
					...
					<div class="row">
						<div class="col-lg-3 subclass">
							...
							<div ng-if="inbox.threadmsg.length!=1">
								<div class="name_left ng-cloak" ng-if="inbox.MI_FROM_ID!=currentUser.UserId">{{ usersListbyID[inbox.MI_FROM_ID].user+", me ("+inbox.threadmsg.length+")" }}</div>
								<div class="name_left ng-cloak" ng-if="inbox.MI_FROM_ID==currentUser.UserId">me, {{ usersListbyID[inbox.MI_TO_ID].user+" ("+inbox.threadmsg.length+")" }}</div>
							</div>
							<div ng-if="inbox.threadmsg.length==1">
								<div class="name_left ng-cloak" ng-if="inbox.MI_FROM_ID!=currentUser.UserId">{{ usersListbyID[inbox.MI_FROM_ID].user }}, me</div>
								<div class="name_left ng-cloak" ng-if="inbox.MI_FROM_ID==currentUser.UserId">me, {{ usersListbyID[inbox.MI_TO_ID].user }}</div>
							</div>
						</div>
						<div class="col-lg-5 subclass">
							<div class="subject"><a href="#" ng-click="uCommunication.mailRead(inbox)">{{inbox.MI_SUBJECT}}</a></div>
						</div>
						<div class="col-lg-1 subclass">
							<div class="date">{{uCommunication.initDate(inbox)}}</div>
						</div>
						<div class="col-lg-3 ">
							<div class="mailbx">
								<div class="res"><img src="support/communication/images/mo1.png" alt=""/></div>
								<!-- <div class="res"><img src="support/communication/images/ar.png" alt=""/></div> -->
								<div class="res"><img src="support/communication/images/tr.png" ng-click="uCommunication.selectedmail(inbox);uCommunication.deleteMail()" alt=""/></div>
							</div>
						</div>
					</div>
				</div>
			</div>
			<!-- <div ng-show="searchvalue=='true'">Some messages might match for your search in :
	...
	<div class="tab-pane active" ng-show="mailread" ng-cloak>
		<div class="panel-group">
			<div class="panel panel-default noborder" ng-repeat="thread in readMail.threadmsg">
					<div class="panel-heading quick" ng-class="$index+1==readMail.threadmsg.length?'opened':''">
						<div class="row">
							...
							<div class="col-sm-2 col-xs-3" style="padding-right:0px"> 
								<div class="dropdown right-drop-btn">
					...
					<div id="collapse-{{$index+1}}" class="panel-collapse collapse" ng-class="$index+1==readMail.threadmsg.length && !replymail ? 'in':''"   ng-cloak>
