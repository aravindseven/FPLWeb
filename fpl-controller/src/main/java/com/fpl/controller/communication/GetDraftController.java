...
public class GetDraftController extends AjaxBaseController{
	@Autowired
	...
	public ModelAndView executeAjaxCall(final HttpServletRequest request, final HttpServletResponse response) {
		...
		try {
			...
			{
				 stats = communicationManager.getDraft(loginInfo.getLoginCredentialid(),Integer.parseInt(request.getParameter("pageno")), filter);
			...
			else
			{
				...
				if(("DRAFT").equalsIgnoreCase(request.getParameter("box")))
				{
