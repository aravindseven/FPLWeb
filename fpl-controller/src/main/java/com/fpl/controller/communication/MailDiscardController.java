...
public class MailDiscardController extends AjaxBaseController {
	@Autowired
	...
	public ModelAndView executeAjaxCall(HttpServletRequest request, HttpServletResponse response) {
		...
		String mids=request.getParameter("mailList");	
				...
				System.out.println("getting userid from discardcontroller"+userid);
		try {
			Boolean stats = communicationManager.Discardfromdraft(mids,userid);
