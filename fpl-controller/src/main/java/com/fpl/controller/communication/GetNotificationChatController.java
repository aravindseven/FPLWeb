...
public class GetNotificationChatController extends AjaxBaseController {
	@Autowired
	...
	public ModelAndView executeAjaxCall(final HttpServletRequest request, final HttpServletResponse response) {
		...
		try {
			...
			{
				System.out.println("calledserch is null");
				stats = communicationManager.getNotify(loginInfo.getLoginCredentialid(),Integer.parseInt(request.getParameter("pageno")),filter);
			...
			else
			{
				...
				System.out.println("A Size of the important arra");
				System.out.println(rec_id.size());*/
				if(("INBOX").equalsIgnoreCase(request.getParameter("box")))
				{
					...
					System.out.println("response from dao fom sent");
					System.out.println(stats);
					map.put("sent", stats);
					...
					stats = communicationManager.getsearchmail(loginInfo.getLoginCredentialid(),request.getParameter("search"),request.getParameter("filter"),"trash",rec_id);
					map.put("trash", stats);*/
