...
public class GetArchiveController extends AjaxBaseController{
	@Autowired
	...
	public ModelAndView executeAjaxCall(final HttpServletRequest request, final HttpServletResponse response) {
		...
		try {
			...
			{
				 stats = communicationManager.getArchive(loginInfo.getLoginCredentialid(),Integer.parseInt(request.getParameter("pageno")),filter);
			...
			else
			{
				...
				if(("ARCHIVE").equalsIgnoreCase(request.getParameter("box")))
				{
