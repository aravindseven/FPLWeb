...
public class ChangeStarController extends AjaxBaseController {
	@Autowired
	...
	public ModelAndView executeAjaxCall(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			String stats = communicationManager.ChangeStarRating(loginInfo.getLoginCredentialid(),request.getParameter("msgid"),request.getParameter("star"),request.getParameter("box"));
